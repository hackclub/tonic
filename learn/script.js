...
document.getElementById("mutant_code_copy").onclick = async function () {
  navigator.clipboard.writeText(
    document.getElementById("mutant_code").innerText
  );
...
class Mutant {
  ...
  async introduction() {
    ...
    await this.content.choice2({
      ...
      callback_a: async () =>
        await this.grinning.say("Only my favorite thing in the world!"),
      callback_b: async () =>
        await this.hand_over_mouth_open_eyes.say("Well, you see..."),
    });
    ...
    await this.thinking.choice2({
      ...
      callback_a: async () =>
        await this.grinning.say(
          "Great! But I'll remind you about it, just in case."
        ),
      callback_b: async () =>
        await this.grinning.say("That's okay! Let me tell you about it..."),
    });
    await this.thinking.say(
      "*Jekyll* is a tool that can help you _build a website_.",
      { image: "jekyll-logo-2x" }
    );
    await this.slight_smile.say(
      "With Jekyll, you can write just a little bit of code...",
      { image: "jekyll-new-code" }
    );
    await this.grinning.say("...and get a beautiful site in return!", {
      image: "jekyll-new",
    });
    await this.slight_smile.say(
      "You just need to pick a *theme* to decide how it looks."
    );
    ...
    await this.slight_smile.say(
      "This one's more yellow, like an old sheet of paper...",
      { image: "parchment" }
    );
    ...
    await this.thinking.say(
      "A Jekyll theme can be as simple or as complex as you want."
    );
    await this.thinking.say(
      "Look at this one: it has some navigation that the other themes don't."
    );
    await this.grin.say("The only limit is your imagination!");
    await this.hand_over_mouth_open_eyes.say(
      "But I don't just want you to pick an existing theme...",
      { image: null }
    );
    ...
    await this.slight_smile.choice2({
      ...
      callback_b: async () =>
        await this.grinning.say("I promise it's not as hard as you think!"),
    });
    ...
    await this.smile_hearts.say(
      "At the end, you'll have a beautiful Jekyll theme published for the world to see!"
    );
  ...
  async introduce_tasks() {
    ...
    await this.slight_smile.say(
      "Every time you come here, you'll see these first."
    );
    await this.slight_smile.say(
      "You'll start at the top, and work your way down the list..."
    );
    await this.grinning.say(
      "...learning more and more about Jekyll as you go!"
    );
    await this.hand_over_mouth_open_eyes.say(
      "I'll always save your progress, so you can keep track of what to do next..."
    );
    await this.hand_over_mouth.say(
      "...and you can always go back to a task you've already finished, too."
    );
  ...
  async complete() {
    ...
    await this.grinning.say(
      "You've completed all the tasks I have for you so far."
    );
    await this.slight_smile.say(
      "Starting from a simple template, you now have the beginnings of your very own Jekyll theme."
    );
    await this.smile_with_tear.say("I knew you could do it!");
    await this.hushed.say("But what about showing it to the world?");
    await this.hand_over_mouth_open_eyes.say(
      "For that, I'll need some more time to collect my thoughts..."
    );
    ...
    document.getElementById("tasks_container").scrollTop =
      document.getElementById("tasks_container").scrollHeight;
    await sleep(1000);
    document.getElementById("tasks_container").classList.remove("in");
  }
  /**
  ...
  async text_entry(object) {
    await new Promise((resolve) => {
      ...
      document
        .getElementById("text_entry_container")
        .classList.remove("hidden-h");
      ...
      document.getElementById("text_entry").onkeydown = async function (e) {
        if (e.key === "Enter") {
          ...
          if (!S.success) {
            ...
            if (S.lost_id) {
              alert(
                "Somehow, Mutant lost track of your user ID! This shouldn't happen. Send a message in #tonic on the Hack Club Slack!"
              );
            }
            return;
          }
...
document.getElementById("music_toggle").onclick = function () {
  ...
  document.getElementById("music_toggle_icon").src = music_enabled
    ? "/assets/ms/speaker_medium_volume.svg"
    : "/assets/ms/speaker_muted.svg";
};
    ...
    fetch("/scraps", { credentials: "include" })
      ...
      .then(async (data) => {
        ...
        if (data.records.length === 0) {
          ...
          const filtered_records = data.records.filter(
            (x, i, r) =>
              r.findIndex((x2) => x2.fields["Task"] === x.fields["Task"]) === i
          );
          for (const record of filtered_records) {
            const task_name = record.fields["Task"];
            const task = Object.values(tasks.all_tasks).find(
          ...
          const tasks_which_should_be_unlocked_based_on_required_tasks =
            Object.values(tasks.all_tasks).filter((task) => {
              if (task.requires_tasks.length === 0) return false;
              if (
                tasks_state[task.name] < 3 &&
                task.requires_tasks.every((t) => tasks_state[t] === 4)
              )
                return true;
              return false;
            });
